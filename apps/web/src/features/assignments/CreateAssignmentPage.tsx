...
export function CreateAssignmentPage({
  ...
  const [submissionInstructions, setSubmissionInstructions] = useState(
    initialValue?.submissionInstructions ?? '',
  )
  const [deadline, setDeadline] = useState(() =>
    initialValue ? toLocalDateTime(new Date(initialValue.deadlineAt)) : '',
  )
  ...
  const canSubmit =
    ...
    submissionInstructions.trim().length > 0 &&
    validDates &&
    !isSubmitting
  ...
  const confirmReminder = (value: string) => {
    if (activeReminderIndex === 'new') {
      ...
      setReminders((current) =>
        current.map((item, index) => index === activeReminderIndex ? value : item),
      )
    }
    ...
    <main className="screen create-assignment-screen">
      ...
      <form className="create-assignment-form" onSubmit={submit}>
        <Field label="제목" required>
          <input maxLength={MAX_TITLE_LENGTH} onChange={(event) => setTitle(event.target.value)} placeholder="과제 제목을 입력해주세요" value={title} />
        </Field>
        <Field label="내용" required>
          <textarea maxLength={MAX_CONTENT_LENGTH} onChange={(event) => setContent(event.target.value)} placeholder="설명을 입력해주세요" value={content} />
        </Field>
        <Field label="제출 방법" required>
          <textarea className="is-short" maxLength={1000} onChange={(event) => setSubmissionInstructions(event.target.value)} placeholder="링크 등 제출 방법을 입력해주세요" value={submissionInstructions} />
        </Field>
        <Field label="마감 시각" required>
          <input min={minimum} onChange={(event) => setDeadline(event.target.value)} type="datetime-local" value={deadline} />
        </Field>
        <fieldset className="assignment-reminders">
          <legend>리마인드 시각 <small>(선택)</small></legend>
          {reminders.map((value, index) => (
            <div className="assignment-date-input" key={`${value}-${index}`}>
              <button
                aria-label={`리마인드 시각 ${index + 1}`}
                className="assignment-reminder-value"
              ...
              <button
                aria-label={`리마인드 ${index + 1} 삭제`}
                className="assignment-reminder-remove"
                onClick={() => setReminders((current) => current.filter((_, itemIndex) => itemIndex !== index))}
                type="button"
              >
          ...
          {reminders.length < 5 ? (
            <button
              aria-label="리마인드 추가"
              className="assignment-add-reminder"
      ...
      {activeReminderIndex !== null ? (
        <ReminderDateTimePicker
          ...
          value={typeof activeReminderIndex === 'number' ? reminders[activeReminderIndex] : undefined}
        />
      ) : null}
    </main>
  )
